<script lang="ts">
        ...
        empresa: parseInt($empresaSeleccionada),
            ...
            dataParametros: [
              ...
              },
              {
                parametro: "@nota",
                tipo: Enu.DbType.String,
                valor: "Proceso de pago dese la web"
              },
              {
                parametro: "@cuentaEmpresa",
          ...
          if(res.exito)
          {
           let resNomina=res.data;
              if(resNomina.length>0)
              {
                ...
                {
                  limpiar();
                  Util.VerMensaje("Atencion","Pago de nomina procesado con exito.");
                  cargarNominasDefinitivasPostRegitro();
                }else{
    ...
    const cargarNominasDefinitivas=async ()=>{
      try{
        if(!ePeriodoNomina)
       {
        let datos:Tipos.RData={
          ...
          if(res.exito)
          {
              ...
              {
                  Util.VerMensaje("Atencion","No se encontraron nominas pendientes de pago.");
              }
             console.log("Exito");
    ...
    const cargarNominasDefinitivasPostRegitro=async ()=>{
      try{
        if(!ePeriodoNomina)
       {
        let datos:Tipos.RData={
          ...
          {
              console.log("error al consultar nominas definitivas");
...
{#if permisosCrud.crud == 1}
   ...
   <main in:fade="{{duration: 1500}}">
    ...
    <TituloPagina Titulo="PROCESO PAGO NOMINA"></TituloPagina>
    
    
    <section class="w-full">
    <article class="rounded-b-2xl rounded-t-2xl p-3 mx-[5%]">
    
      <button on:click={()=>cargarNominasDefinitivas()} class="btn btn-sm mx-1 bg-green-700 font-bold border-none hover:bg-white hover:text-green-700">Consultar Nominas Pendientes De Pago</button>
        ...
        {#if consultaNominas.length>0}
          <section class="flex flex-row-reverse py-2  rounded-b-2xl pr-2">
            <button for="modalCitas" on:click={procesarPagoNomina} class="btn btn-md mx-1 font-bold bg-red-700 border-none hover:bg-white hover:text-green-700">Procesar Pago</button>
