  ...
  let beneficiosEmpleado:Tipos.BeneficiosEmpleados[]=[];
   ...
   const consultaEmpleado=async()=>{
          ...
          if(!eBeneficioEmpleado.beneficio)
    {
     ...
     const asignarBeneficio=async ()=>{
         validaBeneficio();
         validaCedula();
         validaFrecuencia();
         validaMonto();
         try{
          if(!eBeneficioEmpleado.beneficio && !eBeneficioEmpleado.empleado && !eBeneficioEmpleado.frecuencia &&!eBeneficioEmpleado.monto)
    {
      let datos:Tipos.RData={
        empresa: parseInt($empresaSeleccionada),
            sp: "AsignaBeneficioEmpleado",
            modelo: "RDataRegistro",
            dataParametros: [
              {
                parametro: "@cedula",
                tipo: 16,
                valor: fBeneficioEmpleado.empleado.toString()
              },
              {
                parametro: "@concepto",
                tipo: 11,
                valor: fBeneficioEmpleado.beneficio.toString()
              },
              {
                parametro: "@frecuencia",
                tipo: 11,
                valor: fBeneficioEmpleado.frecuencia.toString()
              },
              {
                parametro: "@quincena",
                tipo: 11,
              ...
              },
              {
                parametro: "@valor",
                tipo: 7,
                valor: fBeneficioEmpleado.monto.toString()
              },
              {
                parametro: "@nota",
                tipo: 16,
                valor: fBeneficioEmpleado.nota.toString()
              },
              {
                parametro: "@empleado",
                tipo: 16,
            ...
            let resNomina=res.data;
              if(resNomina.length>0)
              {
               ...
               consultaBeneficiosEmpleado();
                ...
                {
                  //limpiar();
                  Util.VerMensaje("Atencion","Beneficio asignado con exito.");
                }else{
    ...
    const options = {
     ...
     const validaBeneficio=()=>{
       if(fBeneficioEmpleado.beneficio.length<1)
      {
         eBeneficioEmpleado.beneficio=true;
      }else {
         eBeneficioEmpleado.beneficio=false;
      }
     }
...
{#if permisosCrud.crud == 1}
   ...
   <main in:fade="{{duration: 500}}">
    ...
    <TituloPagina Titulo="ASIGNAR BENEFICIO"></TituloPagina>
    ...
    <article class="rounded-b-2xl rounded-t-2xl p-3 mx-[5%]">
        ...
        <section class="pt-0">
          ...
          <div class="form-control mb-1">
            <div class="input-group relative">
              <LabelHorizontal>Beneficio</LabelHorizontal>
          ...
          <div class="form-control mb-1">
            <div class="input-group relative">
              ...
              <select bind:value={fBeneficioEmpleado.frecuencia}  name="cfrecuencia"  class="select select-bordered font-normal text-lg text-gray-500 w-[400px]">
               <option disabled={true} selected={true}></option> 
               {#each $frecuenciaConceptos as c }
                 <option value={c.id}>{c.descripcion}</option> 
               {/each}
               </select>
              {#if eBeneficioEmpleado.frecuencia}
                  <div class="text-xs absolute ml-3 top-0 left-0 block text-red-400">*Campo Requerido*</div>
              {/if}
            </div>
          </div>
          ...
          <div class="form-control mb-1">
            <div class="input-group">
              ...
              <button on:click={()=>asignarBeneficio()} class="btn">Asignar Beneficio</button>
            </div>
          </div>
   
          {#if beneficiosEmpleado.length>0}
          <h1 class="p-2 bg-base-300 text-center text-lg font-medium mt-2">BENEFICIOS ACTUALES</h1>
          <section class="w-full">
              ...
              <table class="table w-full table-compact mt-[15px]">
                ...
                <tbody>
                  ...
                  <tr>
                    <td>
                      <section>
                       <button on:click={()=>onRetirarBeneficio(b.id)} class="w-[32px] h-[32px] text-red-500 hover:text-gray-800 cursor-pointer">
                         <MdDelete />
                       </button>
...
{:else}
   <!-- else content here -->
   <p>No posee permisos para visualizar esta pagina.</p>
{/if}
